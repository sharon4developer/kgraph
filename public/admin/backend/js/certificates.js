$(document).ready(function () {
    ...
    loadDataTableForCertificates();

    if(document.getElementById('certificate-details-table')){

        Sortable.create(document.getElementById('certificate-details-table').getElementsByTagName('tbody')[0], {
            onEnd: function (event) {
                // Get the new order of the rows
                var newOrder = [];
                $('#certificate-details-table tbody tr').each(function () {
                    newOrder.push(table.row(this).data());
                });

                // Pass the new order to the backend (e.g., using AJAX)
                $.ajax({
                    url: $('#route-for-user').val() + '/certificates/update/order', // Replace with your Laravel route URL
                    method: 'POST',
                    data: {
                        order: newOrder
                    },
...
function loadDataTableForCertificates() {
    table = $('#certificate-details-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: $('#route-for-user').val() + '/certificates/show',
        columns: [
                    ...
                    if (row.can_edit) {
                        buttons += `
                            <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"
                                ...
                                href="${$("#route-for-user").val()}/certificates/${row.id}/edit">
...
$('#certificate-add-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/certificates',
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$('#certificate-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = 'Saving...';
        var certificate_id = $(form).find('input[name=certificate_id]').val();
        // Create
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/certificates/' + certificate_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    if (status == 1) {
        text = 'You want to deactivate this certificate!';
        message = 'Deactivated successfully';
    }
    else {
        text = 'You want to activate this certificate!';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/certificates/change/status",
                data: {
                    id: id,
                },
...
function deleteData(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: "Are you sure, do yo want to delete the certificate ?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Yes!',
        cancelButtonText: 'No, cancel!',
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + '/certificates/' + id,
                data: {
                    id: id,
                },
                success: function (data) {
