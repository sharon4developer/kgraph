    ...
    if(document.getElementById('service-details-table')){
        ...
        Sortable.create(document.getElementById('service-details-table').getElementsByTagName('tbody')[0], {
            onEnd: function (event) {
                ...
                $.ajax({
                    url: $('#route-for-user').val() + '/sub-service-point-contents/update/order', // Replace with your Laravel route URL
                    method: 'POST',
                    data: {
                        order: newOrder
                    },
...
function loadDataTableForServices() {
    table = $('#service-details-table').DataTable({
        ...
        serverSide: true,
        "ajax": {
            "url": $('#route-for-user').val() + '/sub-service-point-contents/show',
            "dataType": "json",
            "type": "GET",
            "data": function(d) {
        ...
        columns: [
            ...
            {data: null,
                render:function(row){
                    return `<a class="btn btn-outline-info btn-rounded mb-2 me-4 _effect--ripple waves-effect waves-light" href="` +$("#route-for-user").val() +`/sub-service-point-contents/point-contents/options?service_point_content_id=` +row.id +`" data-bs-toggle="popover" data-bs-trigger="hover" data-bs-original-title="Options" data-bs-placement="top">
            ...
            },
            {
                ...
                render: function (row) {
                    ...
                    return (`<div style="white-space:no-wrap">
                                    <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"  data-bs-toggle="popover" data-bs-trigger="hover" data-bs-original-title="Edit" data-bs-placement="top"  href="` + $("#route-for-user").val() + `/sub-service-point-contents/` + row.id + `/edit">
...
$('#service-add-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/sub-service-point-contents',
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$('#service-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = 'Saving...';
        var service_point_content_id = $(form).find('input[name=service_point_content_id]').val();
        // Create
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/sub-service-point-contents/' + service_point_content_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/sub-service-point-contents/change/status",
                data: {
                    id: id,
                },
...
function deleteData(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + '/sub-service-point-contents/' + id,
                data: {
                    id: id,
                },
                success: function (data) {
                    table.ajax.reload(null, false);
                    if (data == true)
                        showMessage('success', "Service content deleted successfully");
                },
                error: function (data) {
                    showMessage("warning", "Something went wrong...");
                },
