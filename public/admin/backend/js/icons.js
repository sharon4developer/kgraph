$(document).ready(function () {
    ...
    if (document.getElementById("explore-details-table")) {
        Sortable.create(
            document
                .getElementById("explore-details-table")
                .getElementsByTagName("tbody")[0],
            {
                onEnd: function (event) {
                    ...
                    $.ajax({
                        url: $("#route-for-user").val() + "/icons/update/order", // Replace with your Laravel route URL
                        method: "POST",
                        data: {
                            order: newOrder,
                        },
...
function loadDataTableForExplore() {
    table = $("#explore-details-table").DataTable({
        processing: true,
        serverSide: true,
        ajax: $("#route-for-user").val() + "/icons/show",
        columns: [
...
$("#explore-edit-form").validate({
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = "Saving...";
        var icon_id = $(form).find("input[name=icon_id]").val();
        // Create
        $.ajax({
            type: "POST",
            url: $("#route-for-user").val() + "/icons/" + icon_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    if (status == 1) {
        text = "You want to deactivate this icon!";
        message = "Deactivated successfully";
    } else {
        text = "You want to activate this icon!";
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/icons/change/status",
                data: {
                    id: id,
                },
...
function deleteData(id) {
    Swal.fire({
        title: "Are you sure?",
        text: "Are you sure, do yo want to delete the icon ?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Yes!",
        cancelButtonText: "No, cancel!",
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + "/icons/" + id,
                data: {
                    id: id,
                },
                success: function (data) {
                    table.ajax.reload(null, false);
                    if (data == true)
                        showMessage("success", "Icon deleted successfully");
                },
                error: function (data) {
                    showMessage("warning", "Something went wrong...");
                },
