$(document).ready(function () {
    loadDataTableForResourceCategories();

    if (document.getElementById("resource-category-details-table")) {
        Sortable.create(
            document
                .getElementById("resource-category-details-table")
                .getElementsByTagName("tbody")[0],
            {
                onEnd: function (event) {
                    var newOrder = [];
                    $("#resource-category-details-table tbody tr").each(function () {
                        newOrder.push(table.row(this).data());
                    });

                    $.ajax({
                        url: $("#route-for-user").val() + "/resource-categories/update/order",
                        method: "POST",
                        data: {
                            order: newOrder,
                        },
...
function loadDataTableForResourceCategories() {
    table = $("#resource-category-details-table").DataTable({
        processing: true,
        serverSide: true,
        ajax: $("#route-for-user").val() + "/resource-categories/show",
        columns: [
                    ...
                    if (row.can_edit) {
                        buttons += `
                            <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"
                                ...
                                href="${$("#route-for-user").val()}/resource-categories/${row.id}/edit">
...
$("#resource-category-add-form").validate({
    rules: {
        name: { required: true },
    },
    messages: {
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $("#route-for-user").val() + "/resource-categories",
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$("#resource-category-edit-form").validate({
    rules: {
        name: { required: true },
        resource_category_id: { required: true },
    },
    messages: {
        name: "Name field is required",
    },
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = "Saving...";
        var resource_category_id = $(form).find("input[name=resource_category_id]").val();
        $.ajax({
            type: "POST",
            url: $("#route-for-user").val() + "/resource-categories/" + resource_category_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    if (status == 1) {
        text = "You want to deactivate this category!";
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/resource-categories/change/status",
                data: { id: id },
...
function deleteData(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + "/resource-categories/" + id,
                data: { id: id },
                success: function (data) {
                    table.ajax.reload(null, false);
                    if (data == true)
