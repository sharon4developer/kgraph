$(document).ready(function () {
    ...
    loadDataTableForNewsLetter();

    if(document.getElementById('news-details-table')){

        Sortable.create(document.getElementById('news-details-table').getElementsByTagName('tbody')[0], {
            onEnd: function (event) {
                // Get the new order of the rows
                var newOrder = [];
                $('#news-details-table tbody tr').each(function () {
                    newOrder.push(table.row(this).data());
                });

                // Pass the new order to the backend (e.g., using AJAX)
                $.ajax({
                    url: $('#route-for-user').val() + '/news-letter/update/order', // Replace with your Laravel route URL
                    method: 'POST',
                    data: {
                        order: newOrder
                    },
...
function loadDataTableForNewsLetter() {
    table = $('#news-details-table').DataTable({
        processing: true,
        serverSide: true,

        ajax: {
            url: $('#route-for-user').val() + '/news-letter/show',
            dataType: "json",
            type: "GET",
            data: function (d) {
                    ...
                    if (row.can_edit) {
                        buttons += `
                            <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"
                                ...
                                href="${$("#route-for-user").val()}/news-letter/${row.id}/edit">
...
function deleteData(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: "Are you sure, do yo want to delete the news letter ?",
        icon: 'warning',
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + '/news-letter/' + id,
                data: {
                    id: id,
                },
                success: function (data) {
                    table.ajax.reload(null, false);
                    if (data == true)
                        showMessage('success', "News Letter deleted successfully");
                },
                error: function (data) {
