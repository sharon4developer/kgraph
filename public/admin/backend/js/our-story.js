$(document).ready(function () {
    ...
    loadDataTableForOurStory();

    if(document.getElementById('our-story-details-table')){

        Sortable.create(document.getElementById('our-story-details-table').getElementsByTagName('tbody')[0], {
            onEnd: function (event) {
                // Get the new order of the rows
                var newOrder = [];
                $('#our-story-details-table tbody tr').each(function () {
                    newOrder.push(table.row(this).data());
                });

                // Pass the new order to the backend (e.g., using AJAX)
                $.ajax({
                    url: $('#route-for-user').val() + '/our-story/update/order', // Replace with your Laravel route URL
                    method: 'POST',
                    data: {
                        order: newOrder
                    },
...
function loadDataTableForOurStory() {
    table = $('#our-story-details-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: $('#route-for-user').val() + '/our-story/show',
        columns: [
            ...
            },
            {
                ...
                render: function (row) {
                    ...
                    return (`<div style="white-space:no-wrap">
                                    <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"  data-bs-toggle="popover" data-bs-trigger="hover" data-bs-original-title="Edit" data-bs-placement="top"  href="` + $("#route-for-user").val() + `/our-story/` + row.id + `/edit">
...
$('#our-story-add-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/our-story',
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$('#our-story-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = 'Saving...';
        var our_story_id = $(form).find('input[name=our_story_id]').val();
        // Create
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/our-story/' + our_story_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    if (status == 1) {
        text = 'You want to deactivate this our story!';
        message = 'Deactivated successfully';
    }
    else {
        text = 'You want to activate this our story!';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/our-story/change/status",
                data: {
                    id: id,
                },
...
function deleteData(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: "Are you sure, do yo want to delete the our story ?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Yes!',
        cancelButtonText: 'No, cancel!',
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + '/our-story/' + id,
                data: {
                    id: id,
                },
                success: function (data) {
                    table.ajax.reload(null, false);
                    if (data == true)
                        showMessage('success', "Our story deleted successfully");
                },
                error: function (data) {
                    showMessage("warning", "Something went wrong...");
                },
