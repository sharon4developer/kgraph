...
// All Quill initialization code has been removed
...
$('#banner-add-form').validate({
    ...
    submitHandler: function (form, event) {
        // Get content from TinyMCE editors before submission
        if (typeof tinymce !== 'undefined') {
            var subTitleEditor = tinymce.get('sub_title');
            if (subTitleEditor) {
                $('#sub_title').val(subTitleEditor.getContent());
            }
            var descriptionEditor = tinymce.get('description');
            if (descriptionEditor) {
                $('#description').val(descriptionEditor.getContent());
            }
        }
        //
