    ...
    if(document.getElementById('service-details-table')){
        ...
        Sortable.create(document.getElementById('service-details-table').getElementsByTagName('tbody')[0], {
            onEnd: function (event) {
                ...
                $.ajax({
                    url: $('#route-for-user').val() + '/sub-services/update/order', // Replace with your Laravel route URL
                    method: 'POST',
                    data: {
                        order: newOrder
                    },
...
function loadDataTableForServices() {
    table = $('#service-details-table').DataTable({
        ...
        serverSide: true,
        "ajax": {
            "url": $('#route-for-user').val() + '/sub-services/show',
            "dataType": "json",
            "type": "GET",
            "data": function(d) {
            ...
            },
            {
                ...
                render: function (row) {
                    ...
                    return (`<div style="white-space:no-wrap">
                                    <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"  data-bs-toggle="popover" data-bs-trigger="hover" data-bs-original-title="Edit" data-bs-placement="top"  href="` + $("#route-for-user").val() + `/sub-services/` + row.id + `/edit">
...
$('#service-add-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/sub-services',
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$('#service-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = 'Saving...';
        var sub_service_id = $(form).find('input[name=sub_service_id]').val();
        // Create
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/sub-services/' + sub_service_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/sub-services/change/status",
                data: {
                    id: id,
                },
...
function deleteData(id) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + '/sub-services/' + id,
                data: {
                    id: id,
                },
                success: function (data) {
...
$('#seo-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/sub-services/seo',
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
