$(document).ready(function () {
    ...
    loadDataTableForCrew();

    if(document.getElementById('crew-details-table')){

        Sortable.create(document.getElementById('crew-details-table').getElementsByTagName('tbody')[0], {
            onEnd: function (event) {
                // Get the new order of the rows
                var newOrder = [];
                $('#crew-details-table tbody tr').each(function () {
                    newOrder.push(table.row(this).data());
                });

                // Pass the new order to the backend (e.g., using AJAX)
                $.ajax({
                    url: $('#route-for-user').val() + '/crew/update/order', // Replace with your Laravel route URL
                    method: 'POST',
                    data: {
                        order: newOrder
                    },
...
function loadDataTableForCrew() {
    table = $('#crew-details-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: $('#route-for-user').val() + '/crew/show',
        columns: [
                    ...
                    if (row.can_edit) {
                        buttons += `
                            <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"
                                ...
                                href="${$("#route-for-user").val()}/crew/${row.id}/edit">
...
$('#crew-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = 'Saving...';
        var crew_id = $(form).find('input[name=crew_id]').val();
        // Create
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/crew/' + crew_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    if (status == 1) {
        text = 'You want to deactivate this crew!';
        message = 'Deactivated successfully';
    }
    else {
        text = 'You want to activate this crew!';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/crew/change/status",
                data: {
                    id: id,
                },
...
function deleteData(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: "Are you sure, do yo want to delete the crew ?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Yes!',
        cancelButtonText: 'No, cancel!',
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + '/crew/' + id,
                data: {
                    id: id,
                },
                success: function (data) {
                    table.ajax.reload(null, false);
                    if (data == true)
                        showMessage('success', "Crew deleted successfully");
                },
                error: function (data) {
                    showMessage("warning", "Something went wrong...");
                },
