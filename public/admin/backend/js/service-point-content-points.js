...
$(document).on("change", ".option-type", function () {
    ...
    if (type === "paragraph") {
        // $(contentId).html(`
        //     <textarea class="form-control" id="summernote_${optionId}" required placeholder="Enter paragraph..." name="titles[${titleId}][options][${optionId}][content]"></textarea><br>
        //     `);
        // <input class="form-control" type="url" name="titles[${titleId}][options][${optionId}][url]" placeholder="Url">
        const textareaId = `content_${titleId}_${optionId}`;
        $(contentId).html(`
            <textarea class="form-control" id="${textareaId}" name="titles[${titleId}][options][${optionId}][content]" placeholder="Enter paragraph content..."></textarea>
        `);
        setTimeout(() => {
            initTinyMCE('#' + textareaId, TINYMCE_COMPACT_CONFIG);
        }, 100);
    } else if (type === "option") {
        $(contentId).html(`
