$(document).ready(function () {
    ...
    loadDataTableForExplore();

    if(document.getElementById('explore-details-table')){

        Sortable.create(document.getElementById('explore-details-table').getElementsByTagName('tbody')[0], {
            onEnd: function (event) {
                // Get the new order of the rows
                var newOrder = [];
                $('#explore-details-table tbody tr').each(function () {
                    newOrder.push(table.row(this).data());
                });

                // Pass the new order to the backend (e.g., using AJAX)
                $.ajax({
                    url: $('#route-for-user').val() + '/explore/update/order', // Replace with your Laravel route URL
                    method: 'POST',
                    data: {
                        order: newOrder
                    },
...
function loadDataTableForExplore() {
    table = $('#explore-details-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: $('#route-for-user').val() + '/explore/show',
        columns: [
            ...
            },
            {
                ...
                render: function (row) {
                    ...
                    return (`<div style="white-space:no-wrap">
                                    <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"  data-bs-toggle="popover" data-bs-trigger="hover" data-bs-original-title="Edit" data-bs-placement="top"  href="` + $("#route-for-user").val() + `/explore/` + row.id + `/edit">
...
$('#explore-add-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/explore',
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$('#explore-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = 'Saving...';
        var explore_id = $(form).find('input[name=explore_id]').val();
        // Create
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/explore/' + explore_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
function changeStatus(id, status) {
    if (status == 1) {
        text = 'You want to deactivate this explore!';
        message = 'Deactivated successfully';
    }
    else {
        text = 'You want to activate this explore!';
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "POST",
                url: $("#route-for-user").val() + "/explore/change/status",
                data: {
                    id: id,
                },
...
function deleteData(id) {
    Swal.fire({
        title: 'Are you sure?',
        text: "Are you sure, do yo want to delete the explore ?",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonText: 'Yes!',
        cancelButtonText: 'No, cancel!',
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + '/explore/' + id,
                data: {
                    id: id,
                },
                success: function (data) {
                    table.ajax.reload(null, false);
                    if (data == true)
                        showMessage('success', "Explore deleted successfully");
                },
                error: function (data) {
                    showMessage("warning", "Something went wrong...");
                },
