$(document).ready(function () {
    loadDataTableForProcessSteps();

    if (document.getElementById("process-steps-table")) {
        Sortable.create(
            document
                .getElementById("process-steps-table")
                .getElementsByTagName("tbody")[0],
            {
                onEnd: function (event) {
                    // Get the new order of the rows
                    var newOrder = [];
                    $("#process-steps-table tbody tr").each(function () {
...
function loadDataTableForProcessSteps() {
    table = $("#process-steps-table").DataTable({
        processing: true,
        serverSide: true,
        ajax: {
            url: $("#route-for-user").val() + "/process-steps/show",
            dataType: "json",
            type: "GET",
            data: function (d) {
...
$("#process-step-add-form").validate({
    rules: {
        step_number: {
            required: true,
        },
    ...
    messages: {
        step_number: "Step number field is required",
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $("#route-for-user").val() + "/process-steps",
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$("#process-step-edit-form").validate({
    rules: {
        step_number: {
            required: true,
        },
    ...
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = "Saving...";
        var process_step_id = $(form)
            .find("input[name=process_step_id]")
            .val();
...
function changeStatus(id, status) {
    if (status == 1) {
        text = "You want to deactivate this process step!";
        message = "Deactivated successfully";
    } else {
        text = "You want to activate this process step!";
...
function deleteData(id) {
    Swal.fire({
        title: "Are you sure?",
        text: "Are you sure, do yo want to delete the process step ?",
        icon: "warning",
        showCancelButton: true,
        confirmButtonText: "Yes!",
        cancelButtonText: "No, cancel!",
        ...
        if (result.isConfirmed) {
            $.ajax({
                type: "DELETE",
                url: $("#route-for-user").val() + "/process-steps/" + id,
                data: {
                    id: id,
                },
                success: function (data) {
