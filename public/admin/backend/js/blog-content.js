$(document).ready(function () {
    ...
    loadDataTableForBlog();
});

function loadDataTableForBlog() {
    table = $('#blog-details-table').DataTable({
        processing: true,
        serverSide: true,
        ajax: $('#route-for-user').val() + '/blog-contents/show',
        columns: [
                    ...
                    if (row.can_edit) {
                        buttons += `
                            <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"
                                ...
                                href="${$("#route-for-user").val()}/blog-contents/${row.id}/edit">
...
$('#blog-add-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/blog-contents',
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$('#blog-edit-form').validate({
    ...
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $('#route-for-user').val() + '/blog-contents/' + blog_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
