$(document).ready(function () {
    loadDataTableForResourceContent();
});

function loadDataTableForResourceContent() {
    table = $("#resource-content-details-table").DataTable({
        processing: true,
        serverSide: true,
        ajax: $("#route-for-user").val() + "/resource-contents/show",
        columns: [
                    ...
                    if (row.can_edit) {
                        buttons += `
                            <a class="datatable-buttons btn btn-outline-primary btn-rounded mb-2 me-1 _effect--ripple waves-effect waves-light"
                                ...
                                href="${$("#route-for-user").val()}/resource-contents/${row.id}/edit">
...
$("#resource-content-add-form").validate({
    rules: {},
    messages: {},
    errorElement: "span",
    submitHandler: function (form, event) {
        ...
        $.ajax({
            type: "POST",
            url: $("#route-for-user").val() + "/resource-contents",
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
...
$("#resource-content-edit-form").validate({
    rules: {
        resource_content_id: { required: true },
    },
    messages: {},
    errorElement: "span",
    submitHandler: function (form, event) {
        ...
        var current_btn_text = submitButton.html();
        button_loading_text = "Saving...";
        var resource_content_id = $(form).find("input[name=resource_content_id]").val();
        $.ajax({
            type: "POST",
            url: $("#route-for-user").val() + "/resource-contents/" + resource_content_id,
            contentType: false,
            processData: false,
            data: formData,
            cache: false,
