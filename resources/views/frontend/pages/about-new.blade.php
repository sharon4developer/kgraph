...
<div class="min-h-screen">
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
            <div class="opacity-0 animate-fade-in-up" style="animation-fill-mode: forwards;">
                ...
                <p class="text-xl text-slate-300 leading-relaxed max-w-3xl mx-auto mb-8">
                    For over 12 years, we've been helping individuals and families achieve their Canadian 
    ...
    <section class="py-20 bg-slate-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                ...
                <div class="relative flex justify-center lg:justify-end opacity-0 animate-fade-in-right" style="animation-fill-mode: forwards;">
                    @if(isset($whoweare) && $whoweare->count() > 0)
                        @php
                            $firstItem = $whoweare->first();
                            $filePath = $firstItem->type == 2 
                                ? $locationData['storage_server_path'] . $locationData['storage_video_path'] . $firstItem->file
                                : $locationData['storage_server_path'] . $locationData['storage_image_path'] . $firstItem->file;
                        @endphp
                        @if($firstItem->type == 2)
                            <video class="w-full max-w-md h-auto rounded-3xl shadow-2xl" controls>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
                ...
                <div class="order-1 lg:order-2 opacity-0 animate-fade-in-right" style="animation-fill-mode: forwards;">
                    <h2 class="text-3xl font-bold text-white mb-6">Director's Message</h2>
                    ...
                    <div class="mt-6">
                        <p class="text-xl font-semibold text-white">Mathews Benny</p>
                        <p class="text-blue-400">RCIC, CAPIC Member, Commissioner of Oaths (Ontario)</p>
                    </div>
                </div>
            </div>
        </div>
    ...
    <section class="py-20 bg-slate-800">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16 opacity-0 animate-fade-in-up" style="animation-fill-mode: forwards;">
                <h2 class="text-3xl font-bold text-white mb-6">Our Story: Empowering Your Path to Canada</h2>
                <p class="text-lg text-slate-300 leading-relaxed max-w-4xl mx-auto mb-8">
                ...
                <p class="text-lg text-slate-300 leading-relaxed max-w-4xl mx-auto">
                    Our commitment is to equip you with the knowledge, resources, and guidance needed to successfully navigate the complexities of the immigration process. As you take steps toward your Canadian dream, we will be with you every step of the way, celebrating your milestones and providing support when you need it most. Join us and let us help you unlock the door to new opportunities and a fresh start in your new home.
                </p>
            </div>

            <div class="mt-20">
                <h3 class="text-2xl font-bold text-white text-center mb-12">Journey with KGraph</h3>
                <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-8">
                    @foreach([
                        ...
                        ['value' => ($journeyData->customers ?? '7000') . '+', 'label' => 'Customers Served']
                    ] as $index => $stat)
    ...
    <section class="py-20 bg-slate-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-3xl font-bold text-white mb-4">From Vision to Reality: Our Journey</h2>
            </div>

            <div class="grid grid-cols-1 lg:grid-cols-2 gap-12">
                <div class="bg-slate-800 rounded-3xl p-8 border border-slate-700 opacity-0 animate-fade-in-up" style="animation-delay: 0s; animation-fill-mode: forwards;">
                    <div class="flex items-start space-x-4 mb-6">
                        <div class="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center flex-shrink-0">
                            @include('frontend.icons.target', ['class' => 'w-6 h-6 text-white'])
                        </div>
                ...
                <div class="bg-slate-800 rounded-3xl p-8 border border-slate-700 opacity-0 animate-fade-in-up" style="animation-delay: 0.2s; animation-fill-mode: forwards;">
                    <div class="flex items-start space-x-4 mb-6">
                        <div class="w-12 h-12 bg-blue-600 rounded-xl flex items-center justify-center flex-shrink-0">
                            @include('frontend.icons.globe', ['class' => 'w-6 h-6 text-white'])
                        </div>
    ...
    <section class="py-20 bg-slate-900">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-16">
                <h2 class="text-3xl font-bold text-white mb-4">Our Office Locations</h2>
                <p class="text-slate-300 max-w-2xl mx-auto">
                    ...
                    <div class="bg-slate-800 rounded-2xl p-6 border border-slate-700 opacity-0 animate-fade-in-up" style="animation-delay: {{ $index * 0.1 }}s; animation-fill-mode: forwards;">
                        <div class="text-center mb-6">
                            <div class="w-16 h-16 bg-slate-700 rounded-full flex items-center justify-center mx-auto mb-4">
                                @include('frontend.icons.map-pin', ['class' => 'w-8 h-8 text-blue-600'])
                            </div>
                            <h3 class="text-xl font-bold text-white mb-2">{{ $location->location }}</h3>
                        </div>
                        <div class="space-y-4">
                            <div class="flex items-start space-x-3">
                                @include('frontend.icons.map-pin', ['class' => 'w-5 h-5 text-slate-500 mt-0.5 flex-shrink-0'])
                                <p class="text-slate-300 text-sm leading-relaxed">{{ $location->address }}</p>
                            </div>
                            <div class="flex items-center space-x-3">
                                @include('frontend.icons.phone', ['class' => 'w-5 h-5 text-slate-500 flex-shrink-0'])
                                <a href="tel:{{ $location->phone }}" class="text-blue-400 hover:text-blue-300 font-medium transition-colors">{{ $location->phone }}</a>
                            </div>
                            @if($location->email)
                                <div class="flex items-center space-x-3">
                                    @include('frontend.icons.mail', ['class' => 'w-5 h-5 text-slate-500 flex-shrink-0'])
                                    <a href="mailto:{{ $location->email }}" class="text-blue-400 hover:text-blue-300 font-medium transition-colors">{{ $location->email }}</a>
                                </div>
                            @endif
                        </div>
                    </div>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
                @foreach([
                    ...
                    <div class="bg-slate-700 rounded-2xl p-8 text-center border border-slate-600 opacity-0 animate-fade-in-up" style="animation-delay: {{ $index * 0.2 }}s; animation-fill-mode: forwards;">
                        <div class="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center mx-auto mb-4">
                            @include('frontend.icons.award', ['class' => 'w-8 h-8 text-white'])
                        </div>
                        <h3 class="text-xl font-bold text-white mb-2">{{ $cert['title'] }}</h3>
                        <p class="text-slate-300 mb-3">{{ $cert['description'] }}</p>
                        <span class="inline-flex items-center px-3 py-1 bg-blue-900 text-blue-300 text-sm font-medium rounded-full">
                            {{ $cert['credential'] }}
                        </span>
                    </div>
                @endforeach
