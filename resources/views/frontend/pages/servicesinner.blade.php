        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div class="lg:col-span-2">
                    ...
                    @else
                        {{-- Fallback: Show SubServices if no ServicePoints --}}
                        @php
                            $subServices = isset($services->SubService) && $services->SubService->count() > 0 
                                ? $services->SubService->where('status', 1) 
                                : collect([]);
                        @endphp
                        ...
                        <section class="mb-12">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                @foreach($subServices as $item)
                                    <a href="{{ url('sub-service-details/' . $item->slug) }}" class="flex items-start space-x-3 p-4 bg-slate-800 rounded-xl border border-slate-700 hover:bg-slate-700 hover:border-blue-600 transition-all cursor-pointer group">
                                        @include('frontend.icons.check-circle', ['class' => 'w-5 h-5 text-green-600 flex-shrink-0 mt-0.5 group-hover:text-green-500'])
                                        <span class="text-slate-300 group-hover:text-blue-400 transition-colors">{{ $item->title }}</span>
                                    </a>
                                @endforeach
                            </div>
                        </section>
                ...
                <div class="lg:col-span-1">
                    <div class="sticky top-24 space-y-8">
                        ...
                        <div class="bg-slate-800 rounded-2xl shadow-md p-6 border border-slate-700">
                            <h3 class="text-lg font-bold text-white mb-4">Related Services</h3>
                            <div class="space-y-3">
                                @foreach($relatedServices as $relatedService)
                                    <a href="{{ url('service-details/' . ($relatedService->slug ?? $relatedService->id)) }}" class="block p-3 rounded-xl hover:bg-slate-700 transition-colors border border-slate-600">
    ...
    <section class="py-20 bg-slate-900">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="text-center mb-8">
                <h2 class="text-3xl font-bold text-white mb-4">Start Your {{ $services->title ?? 'Service' }} Application</h2>
