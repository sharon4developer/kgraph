...
@section('content')
    ...
    <style>
        ...
        .revamp-services .splide__arrow {
            background: #ffffff;
            border: 1px solid #062358;
            border-radius: 5%;
            height: 4em;
            opacity: 10;
            width: 4em;
    ...
    <div class="revamp-services bg-[#062358]">
        <div class="container mx-auto px-5 lg:px-12 h-full w-full py-8 md:pt-[15%] lg:py-[8%]">
            <div class="splide services-grade py-2 rounded-md my-1 px-4" id="card-carousel">
                <div class="splide__track">
                    <ul class="splide__list">
                        @foreach ($services->ServicePoint as $ServicePoint)
                            <li class="splide__slide card cursor-pointer"
                                data-description="{{ $ServicePoint->description }}">
                                <div class="bg-white w-full py-2 rounded-md my-8">
                                    <h2 class="text-[#072459] font_inter text-[20px] px-4 font-extrabold capitalize">
                                        {{ $ServicePoint->title }}
                                    </h2>
                                </div>
                            </li>
            ...
            <div class="services-inner text-white font_inter py-4" id="service-description">
            ...
            <div>
                ...
                <div
                    ...
                    @foreach ($services->ServiceFaq as $ServiceFaq)
                        <div
                            class="accordion-item bg-white p-5 lg:w-1/2 rounded-xl overflow-hidden cursor-pointer transition-all duration-300">
                            <div class="flex justify-between items-center w-full accordion-header gap-4"
                                onclick="toggleAccordion(this)">
                                <img class="accordion-icon transition-transform duration-300 w-2 lg:w-[14px]"
                                    src="{{ asset('assets/faqplus.png') }}" alt="Plus Icon">
                                <h3 class="text-[12px] font-semibold lg:text-[14px] lg:font-medium">
                                    {{ $ServiceFaq->title }}
                                </h3>
                            </div>
                            <div
    ...
    {{-- <div class="bg-[#062358]">
        <div class="container mx-auto px-5 lg:px-12 h-full w-full py-8 md:pt-[15%] lg:py-[8%]">
            @foreach ($services->ServicePoint as $ServicePoint)
                <div>
                    <div class="services-grade w-full py-2 rounded-md my-8">
                        <h2 class="text-[#072459] font_inter text-[20px] pl-4 font-extrabold capitalize">
                            {{ $ServicePoint->title }}</h2>
                    </div>
    ...
    <div class="dontwait bg-[#062358] h-[30vh] w-full">
        <div class="dontwaitwrpr h-full w-full">
            <div
                ...
                <a href="{{ url('eligibility-check/' . $services->slug) }}"
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // Element where the description will be shown
            const descriptionElement = document.getElementById('service-description');

            // Function to update description based on clicked slide
            ...
            splide.Components.Slides.forEach((slide) => {
                slide.slide.addEventListener('click', function() {
                    const description = slide.slide.getAttribute('data-description');
                    updateDescription(description);
                });
            });

            // Initialize with the description of the first slide
            const initialDescription = splide.Components.Slides.getAt(splide.index).slide.getAttribute(
                'data-description');
