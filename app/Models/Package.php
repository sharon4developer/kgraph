...
class Package extends Model
{
    ...
    protected $fillable = ['country', 'description', 'image', 'intervention_image', 'status','order'];

    public static function getFullData($data)
    {
        $locationData = getLocationData();

        $value =  SELF::select('country', 'image', 'id', 'status', 'created_at')->orderBy('order', 'asc');
    ...
    public static function getFullDataForHome(){
        return SELF::select('image','id','country','description')->orderBy('order','asc')->where('status',1)->get();
    }
