...
class ServiceSeo extends Model
{
    ...
    protected $table = 'service_seos';

    protected $fillable = ['service_id','meta_title','meta_description','meta_keywords','og_title','og_description','og_url','og_image','schema'];
    ...
    public static function getData($service_id){
        ...
        $data = SELF::where('service_id',$service_id)->first(['service_id','meta_title','meta_description','meta_keywords','og_title','og_description','og_url','og_image','schema','id']);
        if($data)
    ...
    public static function createData($data){
        ...
        $value = SELF::where('service_id',$data->service_id)->first();

        if(!$value)
            $value = new ServiceSeo;
    ...
    {
        $locationData = getLocationData();
        $data = Service::where('id', $service_id)->first('id');
        if ($data && $data['seo'])
